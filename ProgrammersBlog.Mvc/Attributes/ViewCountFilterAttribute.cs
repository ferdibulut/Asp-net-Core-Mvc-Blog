...
namespace ProgrammersBlog.Mvc.Attributes
{
    ...
    public class ViewCountFilterAttribute : Attribute, IAsyncActionFilter
    {
        ...
        {
            var articleId = context.ActionArguments["articleId"];//articleId alıyoruz
            if (articleId is not null)
            {
                string articleValue = context.HttpContext.Request.Cookies[$"article{articleId}"]; //key: article22 gibi kontrol ediyoruz
                if (string.IsNullOrEmpty(articleValue))
                {
                    ...
                    await articleService.IncreaseViewCountAsync(Convert.ToInt32(articleId));//görünürlügü artırıyoruz
                    await next();//işleme devam etmesini saglıyoruz
                }
            }
            await next();
        }
